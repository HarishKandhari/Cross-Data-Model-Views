...
rdbms=(getDataSources(getTree("D:\pendrive\ooks\study material\data_modeling\dm_proj\connecting.xml"),'rdbms_datasource','dbname'))
csvinfo=(getDataSources(getTree("D:\pendrive\books\study material\data_modeling\dm_proj\connecting.xml"),'csv_datasource','csvname'))
xmlinfo=(getDataSources(getTree("D:\pendrive\books\study material\data_modeling\dm_proj\connecting.xml"), 'xml_datasource', 'xmlname'))
viewstore = (getDataSources(getTree("D:\pendrive\books\study material\data_modeling\dm_proj\connecting.xml"), 'viewstore', 'dbname'))
...
def generateDataFrames(columns, rdbms, csvinfo, sql):
    ...
    settingDataFrames(df_list,sql)

def settingDataFrames(df_list, sql):

    '''viewsdb = pymysql.connect(host=viewstore['views']["password"],
